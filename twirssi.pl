...
# Bump version so it's at least different!
$VERSION = "2.5.1beta-benh";
%IRSSI   = (
    authors     => 'Dan Boger',
...
sub magic_up_links {
	...
	if ($text =~ m,http://[a-z0-9A-Z\./]+, ) {
        ...
        else
        {
            ...
            foreach my $u (@urls)
            {
                # check that it's actually a return in the JSON, just in
                # case it's not. 
                if ( $json_urls->{$u} =~ m%^http://% )
                {
                    # may as well do /g incase a URL is repeated.
                    $text =~ s/$u/$json_urls->{$u}/g;
                }
            }
        }
    }
